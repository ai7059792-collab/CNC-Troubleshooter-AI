...
def extract_pdf_text(pdf_bytes, password=""):
    ...
    try:
        ...
        # Some manufacturer manuals are encrypted/protected.
        if reader.is_encrypted:
            ...
            if not result:
                raise ValueError(
                    "The PDF password was rejected. Please check the password."
                )
        ...
        for page_number, page in enumerate(reader.pages, start=1):
            try:
                ...
                # Continue with other pages if one page is problematic.
                text = f"[Page {page_number} could not be extracted: {page_error}]"
        ...
        if not pages:
            raise ValueError(
                ...
                "OCR support will be added in a later version."
            )
        ...
        raise RuntimeError(
            "The PDF could not be processed. This can happen with encrypted, "
            "damaged, or unsupported PDFs. Try another PDF or a non-password-protected copy."
        ) from exc
...
manual_password = st.sidebar.text_input(
    "PDF Password (only if required)",
    type="password",
    help="Leave blank for normal PDFs.",
)
...
if st.button("🔧 Diagnose CNC Problem", type="primary", use_container_width=True):
    ...
    if not manual_text.strip():
        st.error("The uploaded PDF could not be read. Check the PDF password, try a non-encrypted PDF, or use a text-based PDF.")
        st.stop()
